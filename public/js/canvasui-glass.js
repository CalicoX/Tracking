var CanvasUIGlass = (() => {
  ...
  // components/canvasui/DockGlass.ts
  var DockGlass_exports = {};
  __export(DockGlass_exports, {
...
void main () {
  ...
  function createGlass(elements, options = {}) {
    ...
    function halfExtents() {
      if (config.pinTo) {
        const r = config.pinTo.getBoundingClientRect();
        return [Math.max(r.width / 2 + 3, 8), Math.max(r.height / 2 + 3, 8)];
      }
    ...
    function syncPinTarget() {
      if (!config.pinTo) return;
      const r = config.pinTo.getBoundingClientRect();
      const out = output.getBoundingClientRect();
      targetX = r.left + r.width / 2 - out.left;
      targetY = r.top + r.height / 2 - out.top;
      if (config.pinAlways) {
        presenceTarget = 1;
        zoomTarget = 1;
      }
    ...
    function render() {
      ...
      const corner = config.pinTo ? Math.min(halfW, halfH) : config.shape === "circle" ? Math.min(halfW, halfH) : Math.min(Math.max(config.corner, 0), Math.min(halfW, halfH));
    ...
    function frame(now) {
      ...
      const kPos = reducedMotion || follow >= 1 ? 1 : 1 - Math.exp(-delta * (4 + follow * 26));
      ...
      render();
      const settled = Math.abs(targetX - posX) < 0.1 && Math.abs(targetY - posY) < 0.1 && Math.abs(zoomTarget - zoom) < 2e-3 && Math.abs(presenceTarget - presence) < 2e-3;
      if (settled && !contentDirty && !(config.pinTo && config.pinAlways)) {
        posX = targetX;
        posY = targetY;
        zoom = zoomTarget;
    ...
    wake = start;
    if (config.pinTo && config.pinAlways) {
      syncPinTarget();
      posX = targetX;
      posY = targetY;
      presence = 1;
    ...
    function onPointerMove(event) {
      ...
      zoomTarget = config.zoom > 1 && target?.closest?.(config.targets) ? Math.min(Math.max(config.zoom, 1), 4) : 1;
      start();
    }
    ...
    if (config.pinTo) observer.observe(config.pinTo);
  ...
  return __toCommonJS(DockGlass_exports);
})();
