...
/* Explore CTAs — colorful border beam (same engine as AI Lab / dock) */
    ...
    (function () {
      ...
      lenis.on("scroll", function () {
        if (window.__updateAiScroll) window.__updateAiScroll();
      });
    })();
    ...
    /* 底栏毛玻璃（无调参面板；默认 frosted + 深色区反色） */
    (function () {
      var tabs = document.getElementById("product-tabs");
      var source = document.getElementById("glass-source");
      var content = document.getElementById("glass-content");
      var output = document.getElementById("glass-output");
      ...
      if (source && content && content.parentNode === source) {
        var shell0 = document.querySelector(".glass-shell");
        if (shell0) shell0.insertBefore(content, source);
      }

      var glass = null;
      try {
        glass = createLiquidGlassDock({
          source: source,
          content: content,
          output: output,
          tabsEl: tabs,
