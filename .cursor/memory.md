...
最后更新：2026-09-22（impact-metrics 进度映射改为进视口即开始，不等钉住）
...
- **ImpactBand**：Loyalty lift 3.3→**3.5**。**进度时机（09-22 Park：进视口就开始，不要钉住再开始）**：impact-metrics `readScrollProgress()` 桌面档 0 点从「section 顶碰视口顶（钉住）」改为「section 顶进视口底」，`scrolled = vh - rect.top`，终点仍是钉住行程的 0.92 处后保持——未钉住时数字/曲线就已在增长。`--impact-p` 无 CSS 消费方，测试只锁模块名。impact-bg-shader 的自转进度（`-rect.top/total`）未动，两模块时机不同是有意的。
