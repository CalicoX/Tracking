...
- **BottomCta → Growing LTV**：eyebrow `Top Global Carriers Coverage` + h2 `Growing LTV along the way` + 副标 + `.bottom-cta-data` 五项数据条（4,000+ Carriers / 9+30 Status / 99.9% Accuracy / 95%+ Recognition / 99.9% SLA）+ 双 CTA。黑底 shader 未动。**文案栏收窄（2026-09-18 Park）**：`.bottom-cta-copy` 桌面 `max-width: 20rem`（标题两行 Growing LTV / along the way，副标跟同宽）；≤1024 叠排 `36rem`；≤768 `none` 吃满内容宽。不要再给 h2 `16ch`（会把 way 孤行）。**≤640 整块居中**（h2/p/`cta-row`）；768 仍左对齐。不要把 `.bottom-cta .cta-row` 在 640 块里再写成 `flex-start`（会盖掉居中）。
