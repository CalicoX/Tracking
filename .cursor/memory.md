...
- 起手贴标题二字，然后 `zoom = 1+p²*360`、半屏内放到最大飞出视口。不要 p³ 拖太久（Park：滚着滚着又往下、要直接放到最大、快一点）。
...
- 不要用 p³ / 1.1 屏把 AI 缩放拖很久（Park：滚着滚着又往下，要直接放到最大、快一点；现 p² × 360、0.48 屏）。
