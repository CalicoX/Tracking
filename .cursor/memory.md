...
- 仍是大「AI」轮廓、滚入汇聚 / 滚出散开、紫罗兰、汇聚后残余扰动、按墨迹盒居中。**不再画 ASCII 方块/加号**，格点改成圆点粒子。实现仍在 `ai-intro-ascii.js` 的 `.ai-intro-aiglyph` 画布，不新开模块。**09-11 细砂定案**：cell 3.2–4、单点无晕、点径约 0.9–1.1（再小会看不见）。**扰动大一点 + 暗一点**：`GLYPH_DRIFT 2.4`（约 8–10px 摆）、色压到 `#8b5cf6`、alpha 再收。字形大小不动。**I 必须是板状衬线**（上下横板 + 中竖，`drawSlabI`），不要 Inter 细棍 I。不要复活 matrix / fillMatrix / 鼠标跟随。
