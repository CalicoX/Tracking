...
- 叠卡预览把四个品牌页按 1120 宽缩放（`--ns-fit` = 卡宽 / 1120）。放大层不缩放。≤640 取消缩放并改单列。NOISSEY 用 Playfair + Poppins，OutIn 用 Hanken Grotesk，SwellPro 用 Poppins，Aussie Betta 用 Playfair + DM Sans。
...
- SwellPro 单独走 `layout: "swellpro"`，对照 `https://store.swellpro.com/apps/17TRACK`。橙公告 Labor Day Sale → 深色店头 → 海滩主图左上 Track Your SwellPro Order（橙胶囊 Official SwellPro Store）→ 白查询卡压底边，按钮 `#FA541C`，单号 `89**********876` → 浅橙结果区：已签收进度到 Delivered、DHL 时间线、Order Information → Support & Services 三张官图 → 运费/税/保修/支持 → 订阅图 + 页脚。
- Aussie Betta 单独走 `layout: "aussie"`，对照 `https://aussiebetta.net/apps/17TRACK`。顶栏 Made in Australia → 绿字标 + Home / All Products / Lanolin / Tallow / Professional Care / Blog → 牧场主图 Track Your Aussie Betta Order → 棕查询卡 `#B8864E`，单号 `11**********45` → 已签收 UniUni 时间线 + Order Information → Recommended For You 四件商品 → Why Aussie Betta 四列 → 牧场故事图 + Natural Ingredients. Scientific Results. → 深棕页脚。底色 `#FAF6EF`。不要再改回 TRAIL / OGL / LUMÉ / NOVA。
...
- **BottomCta → Growing LTV**：eyebrow `Top Global Carriers Coverage` + h2 `Growing LTV along the way` + 副标 + `.bottom-cta-data` 五项数据条（4,000+ Carriers / 9+30 Status / 99.9% Accuracy / 95%+ Recognition / 99.9% SLA）+ 双 CTA。黑底 shader 未动。**居中上下排 + 一行最大宽（2026-09-18 Park）**：`.bottom-cta .section-inner` 全档 column + 居中，文案在上、双 CTA 在下。`.bottom-cta-copy` 桌面 `max-width: 52rem`，标题和副标各一行；≤1024 `min(52rem,100%)`；≤768 `none`（手机副标可换行）。不要再 `20rem` 把标题折成 Growing LTV / along the way，不要左右分栏，不要 768 `flex-start` 盖掉居中。
...
- **CoverageBand（09-08 三轮定稿）**：五项承运商数据独立板块（ExploreMore 后、Growing LTV 前）。整体居中，h2 用 `var(--fs-h2)` **白→蓝从左到右渐变**（#fff→#a8c6ff→#4f8dff），标题-数据间距 56。数字 clamp(34-58px) 静态（计数/blur 动画 Park 否了，会抖）；标签 clamp(14-17px) **桌面 nowrap 一行**。3+2 网格用 **6 列轨道**：上排 3 项各跨 2、下排 2 项各跨 3，两行天然居中；行距 44、max-width 1200。≤640 回 2 列、SLA 跨满。**入场：五项逐个依次上浮**（li 初始 opacity 0 + translateY 28px，`--cv-i` stagger 0.12s/项，band.is-in 由 coverage-globe.js IO 0.25 触发一次；纯 transform 不抖，reduced-motion 直出）。
- **CoverageBand 手机标签换行（2026-09-16 Park，对齐 API）**：≤640 两列时 Standardized / Carrier Recognition 两句 nowrap 溢出。`.coverage-data span` 改 `white-space: normal` + `text-wrap: balance` + `max-width: 100%`。桌面仍 nowrap。文案不改。
...
- 增长曲线手机/平板显示，但高度必须用 px 不用 vh：≤1024 190px / ≤768 280px / ≤640 **200px**、`z-index:2`（盖过地板 veil），这三档都要 `top: auto`（桌面是 top:0 铺满 sticky，漏接会盖住数字）。**形状（09-15 Park）**：顺滑指数（切线连续的三次贝塞尔），左贴底、右冲出顶右；不要折成曲棍球棒，也不要旧缓 S 在 y=56 收平。**面积渐变**：`curve-fill` 只走到 y=0 再封底——路径带负 y 时 objectBoundingBox 把蓝停在视口外，看起来像没填充。线可以负 y 飞出。桌面 `.impact-curve` 铺满 sticky（viewBox 1440×900）；sticky `overflow:hidden` 在顶边裁掉即「飞出屏幕」。
...
- 仍是大「AI」轮廓、滚入汇聚 / 滚出散开、紫罗兰、汇聚后残余扰动、按墨迹盒居中。**不再画 ASCII 方块/加号**，格点改成圆点粒子。实现仍在 `ai-intro-ascii.js` 的 `.ai-intro-aiglyph` 画布，不新开模块。**09-11 细砂定案**：cell 2.8–3.5、单点无晕。A/I 都是板状衬线（`drawSlabA` / `drawSlabI`）。**不要描边提亮**，全身同一 alpha。高度约视口 **80%**。粒子 Park 仍嫌少 → 2026-09-11 再加一档：cell 1.9–2.4（`w/720`）、上限 **16000**（6000 会先截断，只加密格看不见）。滚动 hold 里 **AI → Tracking Page** 变形；Tracking / Page 两行，高度约 80% 视口，不要 140px 封顶。`GLYPH_DRIFT 2.8`（5.4 在密点上太晃，Park 要小一点）。不要 Inter 细棍 I。不要复活 matrix / fillMatrix / 鼠标跟随。
...
- Split-order（09-15 Park：一封邮件一个查询页；邮件「是这样的」= 品牌信那套、**没有单号**；查询页 = 首屏状态卡 + **2 个 Package tab**，跟邮件「2 packages」对齐，不要再做 3 个）。
  ...
  - 查询页：浏览器框 ogl.com/track + Package #1/#2 tab + `.os-status`。Package #1 = In Transit，后两个节点灰。不要查询输入框、不要两张包裹小卡。不要 Package #3。
...
- **AI Lab 案例弹层 ≤640（2026-09-18 Park：里面的案例在移动端都错乱了）**：lightbox 里 `.ogl-status` 桌面是 `1.85fr / 0.8fr`，窄屏会把 Order summary 压成一列字母。≤640 把 status / split / dash / trail / why 全改 `1fr`；`.ogl-mono` 用 `break-word` 禁止 `break-all`；弹层几乎铺满（padding 8）。桌面弹层仍双栏。
...
- 不要给 Features 里的 `.fx-hero-page .browser` 用 hero 那组 `0 64px 100px -36px` 大影（槽垫不够会裁成硬边；跟功能卡同一套 `24px 52px -16px`）。投影垫不要收回 40。
