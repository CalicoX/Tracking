...
最后更新：2026-08-24（Returns 毛玻璃修好；退货方式图标换成钞票/叶子/换货）
...
- Explore 卡**不要 3D hover**。Returns 底图用独立 `<img class="returns-ui-photo">`（`returns-scene.jpg`），浮卡毛玻璃；父级不要 `preserve-3d` / opacity 动画，否则 backdrop-filter 无效。Return method 图标：钞票 / 叶子 / 双向箭头（不要手绘 $）。
