...
- 仍是大「AI」轮廓、滚入汇聚 / 滚出散开、紫罗兰、汇聚后残余扰动、按墨迹盒居中。**不再画 ASCII 方块/加号**，格点改成圆点粒子。实现仍在 `ai-intro-ascii.js` 的 `.ai-intro-aiglyph` 画布，不新开模块。**09-11 Park：太粗 / 不好看 / 再细** → cell 2.4–3、单点无晕、点径约 0.3–0.5，去掉近白/洋红高光。字形大小不动。不要复活 matrix / fillMatrix / 鼠标跟随。
