...
- **亮度/填充定案（「太亮了」→「再暗一点」→「不够明显」→「还是要实心的」）**：最终 = **实心**（整块墨迹格都可见），描边只比内部略亮一点做形体，不再走「只描边」那条路：
  - `alpha = (edge ? GLYPH_EDGE_ALPHA 0.3 : GLYPH_BODY_ALPHA 0.21) + flow * GLYPH_BAND_ALPHA 0.22`，再乘 `flicker * s`
  - `lvl = base*2.4 + flow*0.9 + (edge ? 0.15 : 0)` 只用来选字符档位；`base = (edge ? 0.28 : 0.2) + rand*0.06`
  - 实测：墨迹像素 ≈ 58.5k、平均 alpha ≈ 0.16、峰值 ≈ 0.65（峰值来自字符叠加）。**「不够明显」要的是「实心 + 形状清楚」，不是整体提亮；「太亮了」那版峰值 0.66 但平均更亮。**
  - **再要压暗的话，正确旋钮是把 body 颜色往深了调（如 `#4c3fa8`）而不是继续降 alpha**——降 alpha 会把实心又变回虚的。比 peak 0.66 那版更暗但保持可读。
  - **也不要做「和背景反差」的暗色实体**（我用 `#171236` + 让亮带把覆盖挖开那版被 Park 明确否掉：「不要反差了」）。
