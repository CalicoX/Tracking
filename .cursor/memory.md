...
- 起手贴标题二字，绕 **I 字干** 放大（`1+p²*900`），半屏内让笔画铺满视口。不要绕 AI 中缝放大（缝会越放越大）。
- 缩放跟滚动 **正反都能 scrub**：不要 `peakZoom` 只升不降（Park：没办法倒放回去）。
...
- `test` 本地和远端都已删（2026-08-24）。当前 HEAD `2d03aaf`，与 `origin/main` 一致。
...
- 不要用 `peakZoom` 锁死放大（Park：没办法倒放回去；缩放必须跟滚动正反都能 scrub）。
- 不要绕 AI 二字中缝放大（缝和 A 字洞会越放越大，怎么加比例都铺不满）。绕 I 字干，`1+p²*900`。
- 不要用 p³ / 1.1 屏把 AI 缩放拖很久（Park：滚着滚着又往下，要直接放到最大、快一点；现 p²、0.48 屏）。
- 不要在缩放结束把 overlay 透明度打成 0（Park：突然消失）。回滚由 p 自然缩小。
