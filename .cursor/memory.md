...
- **现用：Bayer 方点 dither** `src/fx/modules/ai-intro-ascii.js`（Park 参考 Closed-Loop 疏密块）。`CELL=7`，全屏铺、不绕标题。**5 个**较小不规则团块。速度约 0.09–0.17 屏/秒。核铺满、外面空。约 40% `+` 按格固定。颜色 `rgba(118,112,148,~0.07–0.17)` 再暗。`is-ascii-on` 藏 hover 标题粒子。
