...
- **亮度定案（「太亮了」→「再暗一点」→ 又「不够明显」）**：最终走**描边提亮 + 内部压暗**两档，总亮度仍很低（实测 >0.35 alpha 的像素只有约 750 个，都在那一圈细描边上）：
  - 描边格 `alpha = GLYPH_EDGE_ALPHA_BASE 0.32 + flow * GLYPH_EDGE_ALPHA_GAIN 0.16`
  - 内部格 `alpha = GLYPH_ALPHA_BASE 0.05 + lvl * GLYPH_ALPHA_GAIN 0.2`，`lvl = base*2.4 + flow*0.9 + (edge ? 0.3 : 0)`，内部 base 0.08–0.13
  - 两档再乘 `flicker * s`。**Park 说「AI 二字不够明显」时，解法是拉开描边/内部的差值，不是整体调亮**——整体一亮就回到「太亮了」。
  - **别再调回 peak 0.66 那一档**（那版就是「太亮了」）；**也不要做「和背景反差」的暗色实体**（我用 `#171236` + 让亮带把覆盖挖开那版被 Park 明确否掉：「不要反差了」）。
