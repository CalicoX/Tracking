...
最后更新：2026-09-18（Growing LTV 全档居中上下排）
...
- **BottomCta → Growing LTV**：eyebrow `Top Global Carriers Coverage` + h2 `Growing LTV along the way` + 副标 + `.bottom-cta-data` 五项数据条（4,000+ Carriers / 9+30 Status / 99.9% Accuracy / 95%+ Recognition / 99.9% SLA）+ 双 CTA。黑底 shader 未动。**文案栏收窄 + 居中上下排（2026-09-18 Park）**：`.bottom-cta .section-inner` 全档 `flex-direction:column` + `align-items:center`，文案在上、双 CTA 在下、整块水平居中。`.bottom-cta-copy` 桌面 `max-width: 20rem`（标题两行 Growing LTV / along the way）；≤1024 `36rem`；≤768 `none`。不要再左右分栏（`1fr auto`），不要 768 `flex-start` 把居中盖掉，不要给 h2 `16ch`（会把 way 孤行）。
