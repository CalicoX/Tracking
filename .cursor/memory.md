...
- **现用：Bayer 方点 dither** `src/fx/modules/ai-intro-ascii.js`（Park 参考 Closed-Loop 疏密块）。`CELL=7`，全屏铺、不绕标题。8 个较小团块，**轮廓要不规则**（拉伸 + 花瓣 + 噪声啃边），不要圆胶囊。速度约 0.09–0.17 屏/秒。核铺满、外面空。约 40% `+` 按格固定。颜色 `rgba(148,140,176,~0.12–0.28)`。`is-ascii-on` 藏 hover 标题粒子。
