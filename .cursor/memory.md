...
- **缩放曲线不用固定 900 倍**（2026-08-27 Park：滚一下突然糊满、缩放没完成）。现用 smoothstep `p²(3-2p)` 插值，终点倍数 `zoomEnd = max(2, diag / (fs * 0.29))`。墨水只有约 0.72em 高、且 A/I 之间有空隙，系数给大了（0.34）p=1 时屏幕四角还没被字形洞盖住、遮罩提前消失（Park：没有滚完就消失了，截图标过左右下角的暗紫底）。0.29 已实测盖住 1280×720 四角。p=1 摘层无缝。断言跟着这条曲线走，别改回 `/p \* p \* 900/`。
