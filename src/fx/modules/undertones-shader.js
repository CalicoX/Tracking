...
(function () {
  ...
  function resize() {
    if (!sizeDirty && canvas.width > 1) {
      gl.viewport(0, 0, canvas.width, canvas.height);
      return;
    }
    var cssW = section.clientWidth;
    var cssH = section.clientHeight;
    if (cssW === lastCssW && cssH === lastCssH && canvas.width > 1) {
      sizeDirty = false;
      gl.viewport(0, 0, canvas.width, canvas.height);
      return;
    }
  ...
  function releaseBackbuffer() {
    /* Full teardown only — not on brief leave-viewport (avoids scroll flicker) */
  ...
  var lastDrawAt = 0;
  /* ~30fps is enough for idle globe spin; frees main thread for Lenis during scroll */
  var FRAME_MS = 33;

  function draw(now) {
    if (!visible || (typeof document !== "undefined" && document.hidden)) {
      /* Pause rAF only — keep last frame in GPU buffer so scrolling past
         hero edge does not flash opacity / 1×1 backbuffer rebuild. */
      running = false;
      return;
    }
  ...
  function start() {
    ...
    running = true;
    /* keep paintedOnce — do not opacity-fade every resume (scroll jitter) */
    requestAnimationFrame(draw);
  }
      ...
      function (entries) {
        entries.forEach(function (e) {
          if (e.isIntersecting) {
            ...
            /* Pause only — releasing the backbuffer mid-scroll causes
               is-ready opacity flicker and layout-adjacent jank in hero. */
            visible = false;
            running = false;
          }
        });
      },
      /* Wider margin + low threshold: avoid edge flicker while leaving hero */
