...
(function (global) {
  ...
  function mountBorderBeam(el, options) {
    ...
    return {
      ...
      /** Full teardown so StrictMode remount can re-init (data-beam guard). */
      destroy: function () {
        if (destroyed) return;
        destroyed = true;
        if (fadeTimer) {
        ...
        var blooms = el.querySelectorAll("[data-beam-bloom]");
        for (var bi = 0; bi < blooms.length; bi++) {
          try {
            if (blooms[bi].parentNode) blooms[bi].parentNode.removeChild(blooms[bi]);
          } catch (e) {
            /* ignore */
          }
        }
        try {
          var styleEl =
            document.querySelector('style[data-border-beam="' + id + '"]') ||
            (styleCache[id] || null);
          if (styleEl && styleEl.parentNode) styleEl.parentNode.removeChild(styleEl);
          delete styleCache[id];
        } catch (e) {
          /* ignore */
        }
      },
  ...
  function mountProductDockBeam() {
    ...
    cleanups.push(function () {
      ...
      // Clear data-beam + bloom so mountProductDockBeam can remount (StrictMode)
      if (api && typeof api.destroy === "function") {
        ...
        tabs.removeAttribute("data-beam");
        tabs.removeAttribute("data-active");
        tabs.removeAttribute("data-fading");
        var leftover = tabs.querySelectorAll("[data-beam-bloom]");
        for (var li = 0; li < leftover.length; li++) {
          try {
            if (leftover[li].parentNode) leftover[li].parentNode.removeChild(leftover[li]);
          } catch (e) {
            /* ignore */
          }
        }
